...
else if b<0 then Just (frak (-a) (-b))
else if gcd a b /= 1 then Just (frak a b)
else Just (Frac a b) where
    frak a b = if gcd a b == 1 then Frac a b else frak (a`div`gcd a b) (b`div`gcd a b)


-- Show instance that outputs Frac n m as n/m
instance Show Fraction where
    show(Frac a b)= if b==0 then error "Illegal fraction" else show a ++ "/" ++show b

-- Ord instance for Fraction
instance Ord Fraction where
    (Frac a b) `compare` (Frac c d) = if b==0 || d==0 then error "Illegal fraction" else (a*d) `compare` (c*b)
